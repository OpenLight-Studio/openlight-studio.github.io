...
export function BilibiliIcon({ size = 16, ...props }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" {...props}>
...
export function ChevronUpIcon({ size = 24, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" width={size} height={size} {...props}>
      <polyline points="18 15 12 9 6 15" />
    </svg>
  );
}

const previewSvgMap: Record<string, (size: number) => JSX.Element> = {
  about: (s) => (
...
export function PreviewIcon({ name, size = 48 }: { name: string; size?: number }) {
  const svg = previewSvgMap[name];
  if (!svg) return null;
  return svg(size);
}
